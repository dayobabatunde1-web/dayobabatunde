...
function SessionCard({ session }: { session: TrackingSession }) {
  ...
  return (
    <div className="rounded-[24px] border border-sky-500/40 bg-blue-900/80 p-5">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <p className="font-semibold text-white">Order {session.id.slice(0, 8).toUpperCase()}</p>
              {order?.status && (
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div className="rounded-[28px] border border-sky-500/40 bg-blue-900/80 p-6 sm:p-8">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-300">Track Orders</p>
          <h1 className="mt-3 text-3xl font-bold text-white sm:text-4xl">Monitor every live tracking link from one focused dashboard.</h1>
          ...
          <div className="mt-6 inline-flex rounded-full border border-sky-500 bg-sky-800 px-4 py-2 text-sm font-semibold text-sky-200">
            {list.length} active link{list.length === 1 ? '' : 's'}
          </div>
        </div>
        <div className="rounded-[28px] border border-sky-500/40 bg-sky-800/90 p-4">
          <TrackListIllustration className="w-full" />
        </div>
      </section>

      {error && (
        <div className="rounded-[24px] border border-rose-400/40 bg-rose-500/20 px-4 py-3 text-sm text-rose-200">
          Error: {error.message}
        </div>
      )}

      {list.length === 0 && !error && (
        <div className="rounded-[24px] border border-dashed border-sky-500/40 bg-sky-800/20 px-6 py-12 text-center text-sky-300">
